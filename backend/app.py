...
print("Caching unprojected graph...")
...
def travel_time_edge(edge_data):
    tt = edge_data.get("travel_time", None)
    if tt is not None:
        return tt
    length_m  = edge_data.get("length", 1.0)
...
def snap_to_walkable_intersection(lat, lon, min_degree=3, max_walk_m=800):
    ...
    while queue:
        ...
        for neighbor in G_proj.neighbors(current):
            ...
            new_dist = dist_so_far + edge_data.get("length", 50.0)
            ...
            if G_proj.degree(neighbor) >= min_degree:
                ...
                nd = G_proj.nodes[neighbor]
                lon_out, lat_out = t.transform(nd["x"], nd["y"])
...
def compute_path_stats(path, num_students):
    total_carbon = total_dist = total_time = 0.0
    for i in range(len(path) - 1):
        u, v = path[i], path[i+1]
        if G_proj.has_edge(u, v):
            data = min(G_proj[u][v].values(), key=lambda d: d.get("length", 1e9))
            eu, ev = NODE_ELEV.get(u, 0), NODE_ELEV.get(v, 0)
...
def quadrant_clusters(stop_coords, n_buses):
    ...
    while len(clusters) < n_buses:
        ...
        if len(largest) < 2:
            break
        sub = np.array([[stop_coords[i][0], stop_coords[i][1]] for i in largest])
        lbl = KMeans(n_clusters=2, n_init=10, random_state=42).fit_predict(sub)
        a = [largest[i] for i in range(len(largest)) if lbl[i] == 0]
        b = [largest[i] for i in range(len(largest)) if lbl[i] == 1]
...
def build_result(buses, total_carbon):
    ...
    saved     = round(total_fastest - total_carbon, 3)
    saved_pct = round(100 * saved / total_fastest, 1) if total_fastest > 0 else 0.0
    return {
        "buses":            buses,
        "num_buses":        len(buses),
...
@app.route("/nodes")
def get_nodes():
    return jsonify(nodes[["x", "y"]].to_dict(orient="records"))
...
def get_topography():
    with rasterio.open(DEM_PATH) as src:
        ...
        shaded = (np.sin(al)*np.sin(slope)
                  + np.cos(al)*np.cos(slope)*np.cos(az - aspect))
        small  = (255 * (shaded - shaded.min()) /
                  (shaded.max() - shaded.min())).astype(np.uint8)
        bounds = {"left": src.bounds.left, "bottom": src.bounds.bottom,
                  "right": src.bounds.right, "top": src.bounds.top}
    return jsonify({"elevations": small.tolist(), "bounds": bounds})
